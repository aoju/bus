...
quartz.choose = function(){
	var dlg = new Dialog({
		...
		title:$.i18n.get('com.ukettle.iQuartz.Verify.Quartz.Schedule.Executor'),
...
quartz.exists = function() {
	...
	if (!group) {
		alert($.i18n.get('com.ukettle.iQuartz.Verify.Quartz.Schedule.Group.Choose'));
		return false;
	}
	var title = $.trim($("#title").val());
    if(!title || !plugin.isWidthChar(title)){
      alert($.i18n.get('com.ukettle.iQuartz.Verify.Quartz.Schedule.Title'));
	  return false;  
    }
    $.post(ctx+"/Widget/Quartz/exists?title="+title, function(data) {
    	if(data =='1'){
    	      alert($.i18n.get('com.ukettle.iQuartz.Verify.Quartz.Schedule.Title.Exist'));
    		  return false;  
    	}
	});
};
$(function() {
	...
	$("#iQuartz").click(function() {
	    ...
	    $.ajax({
	        ...
	        success: function(data){
	    	    ...
	    	    if(!$.trim(executor)){
	    	      alert($.i18n.get('com.ukettle.iQuartz.Verify.Quartz.Schedule.Executor.Choose'));
	    		  return false;  
	    	    }
    			...
    			for (i = 0; i < argK.length; i++) {
    				...
    				if (!argVal) {
    					alert($.i18n.get('com.ukettle.iQuartz.Verify.Quartz.Schedule.Parameters.A') +" ‘" + argKey +"’ " + $.i18n.get('com.ukettle.iQuartz.Verify.Quartz.Schedule.Parameters.B'));
    					return false;
    				}
    			}
	    		$('#formId').submit();
	...
	$("#iSearch").click(function() {
	    ...
	    if(!$.trim(title) && !$.trim(group)){
	      alert($.i18n.get('com.ukettle.iQuartz.Verify.Quartz.Schedule.Search'));
		  return false;  
	    }
	});
});