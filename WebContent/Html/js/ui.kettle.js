...
kettle.loading = function(name){
	$.post(ctx+"/Widget/Kettle/Repos/Loading?name="+name, function(data) {
		...
		alert($.i18n.get('com.ukettle.iQuartz.Verify.Success'));
	}).error(function(data) {
		alert($.i18n.get('com.ukettle.iQuartz.Verify.Error'));
	});
};
/**断开指定资源库*/
...
kettle.choose = function(){
	...
	if (!title) {
		alert($.i18n.get('com.ukettle.iQuartz.Verify.Kettle.Repository.Choose'));
		return false;
	}
	var dlg = new Dialog({
	    type:'iframe',
		...
		title:$.i18n.get('com.ukettle.iQuartz.Verify.Kettle.Repository'),
...
kettle.outs = function(id){
    var dlg = new Dialog({
		...
		title : $.i18n.get('com.ukettle.iQuartz.Verify.Kettle.Monitor.Out'),
		closeText : '<i class="icon-remove"></i>',
	    width : '600px',
	    height : '200px'
	 });
...
kettle.logs = function(id){
    var dlg = new Dialog({
		...
		title : $.i18n.get('com.ukettle.iQuartz.Verify.Kettle.Monitor.Log'),
		closeText : '<i class="icon-remove"></i>',
	    width : '600px',
	    height : '390px',
	    timeOut : 500
...
$(function(){
	$("#iKettle").click(function() {
		...
		for (i = 0; i < argK.length; i++) {
			...
			if (!argVal) {
				alert($.i18n.get('com.ukettle.iQuartz.Verify.Kettle.Repository.Parameters.A')+" ‘" + argKey +"’ " + $.i18n.get('com.ukettle.iQuartz.Verify.Kettle.Repository.Parameters.B'));
				return false;
			}
		}
		$('#formId').submit();
