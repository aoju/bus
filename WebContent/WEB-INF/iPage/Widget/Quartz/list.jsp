...
<div id="holder">
    <div id="container">
         ...
         <div id="main">
         	<h3 style="float:left"><spring:message code="com.ukettle.Quartz.List" /></h3>
	        <div style="float:right; margin:4px 0 0 0">
	         	<form action="${ctx}/Widget/Quartz/List" method="post">
	         	<spring:message code="com.ukettle.Quartz.List.s.Group" /> <select id="group" name="group">
						<option value=""><spring:message code="com.ukettle.Quartz.List.s.Group.Choose" /></option>
						<c:forEach var="g" items="${List}">
							<option id="${g.id}" value="${g.name}" <c:if test="${g.name==group}">selected</c:if>>${g.name}</option>
						</c:forEach>
					  </select>
		  		 <spring:message code="com.ukettle.Quartz.List.s.Title" /> <input type="text" class="text-long" id="title" name="title" value="${title}"/>
		 		 <input type="submit" id="iSearch" value="<spring:message code="com.ukettle.Quartz.List.s.Search" />" />
			  </form>
		 	</div>
		 <table>
		    <tr style="text-align:left;">
				<td><spring:message code="com.ukettle.Quartz.List.Title" /></td>
				<td><spring:message code="com.ukettle.Quartz.List.Group" /></td>
				<td><spring:message code="com.ukettle.Quartz.List.Type" /></td>
				<td><spring:message code="com.ukettle.Quartz.List.Parameters" /></td>
				<td><spring:message code="com.ukettle.Quartz.List.status" /></td>
				<td><spring:message code="com.ukettle.Quartz.List.PreviousTime" /></td>
				<td><spring:message code="com.ukettle.Quartz.List.NextTime" /></td>
			...
			<tr <%if(i%2==0){%>class="odd"<%}%>>
				...
				<td class="action">
				    <a href="${ctx}/Widget/Quartz/Execute?id=${jobDetail.key.toString()}" title="<spring:message code="com.ukettle.Action.Execute" />"><i class="icon-expand"></i></a>
				    <a href="${ctx}/Widget/Quartz/Pause?id=${jobDetail.key.toString()}" title="<spring:message code="com.ukettle.Action.Pause" />"><i class="icon-pause"></i></a>
				    <a href="${ctx}/Widget/Quartz/Resume?id=${jobDetail.key.toString()}" title="<spring:message code="com.ukettle.Action.Resume" />"><i class="icon-undo"></i></a>
				    <a href="${ctx}/Widget/Quartz/View?id=${jobDetail.key.toString()}" title="<spring:message code="com.ukettle.Action.View" />"><i class="icon-eye-open"></i></a>
					<a href="${ctx}/Widget/Quartz/Update?id=${jobDetail.key.toString()}" title="<spring:message code="com.ukettle.Action.Update" />"><i class="icon-pencil"></i></a>
					<a href="${ctx}/Widget/Quartz/Delete?id=${jobDetail.key.toString()}" title="<spring:message code="com.ukettle.Action.Delete" />"><i class="icon-remove"></i></a>
				</td>
			</tr><% i++; %>
			</c:forEach>
		 </table>
