...
/**
 * OfficeProcessManagerPoolEntry负责执行通过{@link LocalOffice}提交的任务。
 ...
 * @since JDK 1.8+
 */
public class OfficeProcessPoolEntry extends AbstractOfficePoolEntry {
     ...
     */
    public OfficeProcessPoolEntry(final UnoUrl unoUrl) {
        this(unoUrl, new OfficeProcessManagerEntryConfig());
    }
     ...
     * @param config 配置.
     */
    public OfficeProcessPoolEntry(
            final UnoUrl unoUrl, final OfficeProcessManagerEntryConfig config) {
        super(config);
        ...
        // 监听到office实例的任何连接事件.
        officeProcessManager.getLocalConnect().addConnectionEventListener(connectionEventListener);
    }

    @Override
    public void doExecute(final MadeInOffice task) throws InstrumentException {

        final OfficeProcessManagerEntryConfig entryConfig =
                (OfficeProcessManagerEntryConfig) config;
        ...
        task.execute(officeProcessManager.getLocalConnect());
    }
    ...
    public boolean isRunning() {
        return super.isRunning() && officeProcessManager.getLocalConnect().isConnected();
    }
    ...
    public void doStart() throws InstrumentException {
        ...
        if (entryConfig.isDisableOpengl()
                && disableOpengl(officeProcessManager.getLocalConnect().getComponentContext())) {
