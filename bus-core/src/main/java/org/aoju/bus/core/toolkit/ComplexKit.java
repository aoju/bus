...
public class ComplexKit {
    ...
    public static int check(String passwd) {
        ...
        if (len % 2 == 0) { // aaabbb
            ...
            if (CharKit.isBlank(part1.replace(part1.charAt(0), Symbol.C_SPACE))
                    && CharKit.isBlank(part2.replace(part2.charAt(0), Symbol.C_SPACE))) {
                level--;
            }
        }
        if (len % 3 == 0) { // ababab
        ...
        if (CharKit.isBlank(passwd.replace(passwd.charAt(0), Symbol.C_SPACE))) {
            level = 0;
        }
