package org.ukettle.basics.shiro.extend.session;
...
public class DefaultSessionIdGenerator implements Comparable<DefaultSessionIdGenerator>, Serializable,
		SessionIdGenerator {
	...
	public static String id() {
		return new DefaultSessionIdGenerator().toString();
	}
	 ...
	 * @return the new id
	 */
	public static DefaultSessionIdGenerator get() {
	 ...
	 */
	public static DefaultSessionIdGenerator massageToObjectId(Object o) {
		if (o == null)
			return null;

		if (o instanceof DefaultSessionIdGenerator)
			return (DefaultSessionIdGenerator) o;

		if (o instanceof String) {
			String s = o.toString();
			if (isValid(s))
				return new DefaultSessionIdGenerator(s);
		}

		return null;
	}

	public DefaultSessionIdGenerator(Date time) {
		this(time, _genmachine, _nextInc.getAndIncrement());
	}

	public DefaultSessionIdGenerator(Date time, int inc) {
		this(time, _genmachine, inc);
	}

	public DefaultSessionIdGenerator(Date time, int machine, int inc) {
	 ...
	 */
	public DefaultSessionIdGenerator(String s) {
		this(s, false);
	}
	...
	public DefaultSessionIdGenerator(byte[] b) {
		if (b.length != 12)
	 ...
	 *            incremental value
	 */
	public DefaultSessionIdGenerator(int time, int machine, int inc) {
		_time = time;
		_machine = machine;
		_inc = inc;
		_new = false;
	...
	public boolean equals(Object o) {
		...
		DefaultSessionIdGenerator other = massageToObjectId(o);
		if (other == null)
			return false;
	...
	public int compareTo(DefaultSessionIdGenerator id) {
		if (id == null)
			return -1;
	...
	static {
		try {
			...
			final int processPiece;
			{
				...
				ClassLoader loader = DefaultSessionIdGenerator.class.getClassLoader();
