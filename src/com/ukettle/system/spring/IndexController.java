...
public class IndexController extends BasicController {
	...
	private String exception(HttpServletRequest request) {
		...
		String message = uLocale.getMessage("com.chamago.iQuartz.Shiro.Logged",
				defaultLocale);
		if (error != null) {
			if ("org.apache.shiro.authc.UnknownAccountException".equals(error)) {
				message = uLocale.getMessage("com.ukettle.Shiro.Unknown",
						defaultLocale);
			} else if ("org.apache.shiro.authc.IncorrectCredentialsException"
					.equals(error)) {
				message = uLocale.getMessage("com.ukettle.Shiro.Pass.Error",
						defaultLocale);
			} else if ("com.ukettle.shiro.CaptchaException".equals(error)) {
				message = uLocale.getMessage("com.ukettle.Shiro.Captcha",
						defaultLocale);
			} else if ("org.apache.shiro.authc.AuthenticationException"
					.equals(error)) {
				message = uLocale.getMessage("com.ukettle.Shiro.Failed",
						defaultLocale);
			} else if ("org.apache.shiro.authc.DisabledAccountException"
					.equals(error)) {
				message = uLocale.getMessage("com.ukettle.Shiro.Disabled",
						defaultLocale);
			}
		}
		return message;
