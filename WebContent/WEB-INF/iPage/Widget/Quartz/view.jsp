     	 ...
     	 <div class="left">
             ...
             <div id="params">
         	 ...
         	 <p><label><spring:message code="com.ukettle.Quartz.Trigger" /></label>
				<input type="radio" id="triggerType" name="triggerType" value="simple" <c:if test="${Entity.triggerType=='simple'}">checked</c:if> onclick="quartz.trigger();" disabled/><spring:message code="com.ukettle.Quartz.Trigger.Simple" />&nbsp;&nbsp;
				<input type="radio"  id="triggerType" name="triggerType" value="cron"  <c:if test="${Entity.triggerType=='cron'}">checked</c:if> onclick="quartz.trigger();" disabled/><spring:message code="com.ukettle.Quartz.Trigger.Cron" />
			 </p>
			 <div id="simpleTrigger">
				...
				<c:if test="${Entity.triggerType=='simple'}">
         	     ...
         	     <p><label class="inline"><spring:message code="com.ukettle.Quartz.Trigger.Frequency" /></label><input type="number"  max="20" data-error-type="inline" required style="padding-left: 66px;" id="frequency" name="frequency" class="text-long" value="${intervalTime}" readOnly/> <spring:message code="com.ukettle.Quartz.Trigger.Tips.Minute" /></p>
	         	 <p><label class="inline"><spring:message code="com.ukettle.Quartz.Trigger.Quantity" /></label><input type="number" max="20" data-error-type="inline" required style="padding-left: 66px;" id="quantity" name="quantity" class="text-long" value="<c:out value='${repeatCount}' default='-1' />" readOnly/> <spring:message code="com.ukettle.Quartz.Trigger.Tips.Continuous" /></p>
         	 </div>
             ...
             <input type="reset" onClick="window.history.go(-1);" value="<spring:message code="com.ukettle.Quartz.Back" />" />
             </div>
