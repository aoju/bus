...
public class UploadFile {
    ...
    public File write(String destPath) throws IOException {
        if (data != null || tempFile != null) {
            return write(FileUtils.touch(destPath));
        }
        return null;
    }
    ...
    public File write(File destination) throws IOException {
        ...
        if (data != null) {
            FileUtils.writeBytes(data, destination);
            data = null;
        } else {
            if (tempFile != null) {
                FileUtils.move(tempFile, destination, true);
            }
        }
        return destination;
    }
    ...
    protected boolean processStream(MultipartRequestInputStream input) throws IOException {
        if (!isAllowedExtension()) {
            // 非允许的扩展名
            Logger.debug("Forbidden uploaded file [{}]", this.getFileName());
        ...
        // 处理硬盘文件
        tempFile = FileUtils.createTempFile(TMP_FILE_PREFIX, TMP_FILE_SUFFIX, FileUtils.touch(setting.tmpUploadPath), false);
        ...
        try {
            ...
            if (size > maxFileSize) {
                ...
                tempFile.delete();
                tempFile = null;
                Logger.debug("Upload file [{}] too big, file size > [{}]", this.getFileName(), maxFileSize);
    ...
    private boolean isAllowedExtension() {
        ...
        String fileNameExt = FileUtils.extName(this.getFileName());
    ...
    private void assertValid() throws IOException {
        if (!isUploaded()) {
            throw new IOException(StringUtils.format("File [{}] upload fail", getFileName()));
        }
    }
